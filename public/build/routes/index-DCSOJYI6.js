...
var GW2ItemFindProcess = class {
  ...
  async SearchOnAccount(account2, itemIds) {
    return new Promise((resolve, error) => {
      let resolvePromises = [];
      let searches = [];
      for (let i = 0; i < itemIds.length; i++) {
        ...
        searches.push({ searchItem: newSearchItem, hits: account2.FindItemInAccount(parseInt(id)) });
      }
...
var useAPIData = (apikey, itemid) => {
  ...
  return { account, isLoading, result, searchItem, isError, Error: Error2 };
};
var ItemSearch = (props) => {
  const { account: account2, isLoading, result, searchItem, isError, Error: Error2 } = useAPIData(props.account, props.itemID);
 ...
 */
//# sourceMappingURL=/build/routes/index-DCSOJYI6.js.map
