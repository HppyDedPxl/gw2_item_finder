...
var GW2ItemFindProcess = class {
  ...
  async Execute() {
    return new Promise((resolve, error) => {
      let apiCall = new GW2API_Call(this.account);
      apiCall.GetCharacterList().then((result) => {
        ...
        for (let i = 0; i < characters2.length; i++) {
          ...
          const fetchEqTempaltes = new GW2API_Call(this.account).GetEquipmentTemplates(characterName);
        ...
        return Promise.all(pendingTemplates).then((results) => {
          ...
          for (let j = 0; j < results.length; j++) {
            ...
            for (let k = 0; k < templateResult.TemplateList.length; k++) {
              ...
              const fetchEquipment = new GW2API_Call(this.account).GetEquipmentFromTemplate(templateResult.CharacterName, templateID);
          ...
          return Promise.all(pendingPopulatedEquipmentTemplates).then((eqData) => {
            ...
            accountInfo.resolveCharactersFromAPI(this.account).then((res) => {
 ...
 */
//# sourceMappingURL=/build/routes/index-OBPY7ICE.js.map
