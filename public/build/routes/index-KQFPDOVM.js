...
var useAPIData = (account, itemid) => {
  ...
  (0, import_react4.useEffect)(() => {
    setLoading(true);
    let FindProcess = new GW2ItemFindProcess(account, itemid);
    let item = new GW2Item(itemid);
    item.populateFromAPI().then((res) => {
      setSearchItem(res);
      let FindProcess2 = new GW2ItemFindProcess(apikey, itemid);
      FindProcess2.Execute().then((res2) => {
        let results = [];
        setResult(res2);
        setLoading(false);
      }).catch((err) => {
 ...
 */
//# sourceMappingURL=/build/routes/index-KQFPDOVM.js.map
