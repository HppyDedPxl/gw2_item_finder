...
var GW2API_Call = class {
  ...
  GetEquipmentTemplates(characterName) {
    return new Promise((resolve, error) => {
      ...
      this.call().then((res) => {
        const result = { CharacterName: characterName, TemplateList: Array.from(res) };
        resolve(result);
      }).catch((err) => error(err));
    });
  }
  ...
  async Execute() {
    return new Promise((resolve, error) => {
      ...
      apiCall.GetCharacterList().then((result) => {
        ...
        Promise.all(pendingTemplates).then((results) => {
...
//# sourceMappingURL=/build/routes/index-KR45T5KB.js.map
