  ...
  async SearchOnAccount(itemIds) {
    return new Promise((resolve, error) => {
      ...
      for (let i = 0; i < itemIds.length; i++) {
        ...
        searches.push({ searchItem: newSearchItem, hits: this.account.FindItemInAccount(parseInt(id)) });
      }
...
var useAPIData = (account, itemid) => {
  ...
  (0, import_react4.useEffect)(() => {
    ...
    let item = new GW2Item(parseInt(itemid));
    item.populateFromAPI().then((res) => {
      setSearchItem(res);
      let FindProcess = new GW2ItemFinder(apikey, itemid);
 ...
 */
//# sourceMappingURL=/build/routes/index-KXEXDQP7.js.map
